...
export class BluetoothTerminal {
     ...
     * @param {function} receiveCallback - function that receives messages from device
     * @param {!(number|string)} serviceUuid - Service UUID
     * @param {!(number|string)} characteristicUuid - Characteristic UUID
     * @param {string} [receiveSeparator='\n'] - Receive separator
     * @param {string} [sendSeparator='\n'] - Send separator
     */
    constructor(receiveCallback, serviceUuid, characteristicUuid,
                receiveSeparator = '\n', sendSeparator = '\n') {
    ...
    _requestBluetoothDevice() {
        ...
        if (this._serviceUuid) {
            requestConfig.filters = [{services: [this._serviceUuid]}]
        } else {
            requestConfig.acceptAllDevices = true;
        }

        return navigator.bluetooth.requestDevice(requestConfig).then((device) => {
